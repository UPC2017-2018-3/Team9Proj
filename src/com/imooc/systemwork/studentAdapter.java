...
import webservice.Bydeletestudent;
import webservice.Bystudentlist;
...
public class studentAdapter extends BaseAdapter{
	...
	public studentAdapter(Context context, ArrayList<ApkEntity> apk_list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.student_listview, null);
			holder.student_t1 = (TextView) convertView
					.findViewById(R.id.student_t1);
			holder.course_b1 = (Button) convertView
					.findViewById(R.id.student_b1);
			convertView.setTag(holder);
		}else{
			holder = (ViewHolder) convertView.getTag();
		}
		holder.student_t1.setText(entity.getstudent_t1());
	...
	class ListOnClick implements OnClickListener{
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			studentAdapter.this.notifyDataSetChanged(); //记得调用它
			String organization = ApkEntity.getorganization();
			String strParams = "organization:" + organization;
			List<Bystudentlist> lgroup=Bystudentlist.listGroup(strParams);
			account=lgroup.get(position).studentaccount;
			String str = "account:" + account;
			String retString=Bydeletestudent.LogSelect(str);
